...
2.  **Server Workflow Engine (`lib/server-workflow-engine.ts`)**: A Node.js-compatible version of the workflow engine that does _not_ depend on React hooks.
...
2.  **Trigger Run**:
    ...
    - **Async Execution**:
        ...
        - _Note_: In a serverless env (Vercel), we might need to keep the request open or use a background worker. For this plan, we assume a long-running request or a robust environment. We will implement a "Process Chunk" approach if needed, but standard concurrency control is the goal.
...
1.  Create `lib/services/batch-service.ts`.
    ...
    - Implement concurrency logic:
        ...
        await Promise.all(
            inputs.map((input) => limit(() => runSingleFlow(input))),
        );
        ```
2.  Create API routes:
...
- **Timeouts**: If running on Vercel, we must be careful.
    - _Mitigation_: We can process in "chunks" if needed, but for "dozens", a single long request might survive if < 60s.
    - _Better_: If the user has a custom server, this works perfectly.
