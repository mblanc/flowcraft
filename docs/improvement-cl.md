...
const isEditor = !!session?.user?.email &&
  sharedWith?.some(s => s.email === session.user?.email && s.role === "edit");
...
2. **Add type guards** instead of `as unknown as T`:
   ...
   function isLLMData(data: unknown): data is LLMData {
     return typeof data === 'object' && data !== null && (data as any).type === 'llm';
   }
   ```
...
const nodeDataMap = useFlowStore(
  useShallow((state) => {
    const map = {};
    state.nodes.forEach((n) => { map[n.id] = n.data; });
...
4. **Include context in error messages**:
   ```typescript
   logger.error(`Error executing node "${node.data.name}" (${nodeId}):`, error);
   ```
