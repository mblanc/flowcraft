...
- **text → 3D asset** _(to add)_ — Generate 3D mesh/model from text
- **image → 3D asset** _(to add)_ — Reconstruct 3D from reference images
...
- **Content classification (NSFW, brand)** _(to add)_ — Classify content against safety and brand categories
- **Audio source separation** _(to add)_ — Isolate vocals, instruments, noise from mixed audio
...
- **Lip sync** _(to add)_ — Synchronize mouth movements to audio
- **Face swap** _(to add)_ — Replace one face with another
- **Motion tracking** _(to add)_ — Track object movement across frames
...
- **Version history** _(to add)_ — Track iterations and enable rollback
- **Render queue** _(to add)_ — Manage async long-running jobs with priority
...
- **Novel composition**: the planner can compose _new_ workflows by reasoning over the primitive catalog, not just selecting from templates. This is where the Gemini 3.1 backbone earns its keep.
