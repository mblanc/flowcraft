...
**Design rule:** A subagent should be able to handle any task within its domain without calling back to the orchestrator for planning help. The orchestrator defines *what* needs to happen; the subagent decides *how*. If a subagent consistently needs orchestrator guidance for routine tasks, its system prompt needs more domain knowledge.
...
interface PrimitiveAdapter<TInput, TOutput> {
  ...
  // Execution
  execute(input: TInput, provider?: ProviderConfig): AsyncGenerator<Progress, TOutput>
...
```typescript
function selectProvider(input: TInput, constraints: TaskConstraints): ProviderConfig {
...
const textToImageAdapter: PrimitiveAdapter<TextToImageInput, ImageOutput> = {
  ...
  inputSchema: z.object({
    ...
    style_preset: z.enum(["photo", "illustration", "3d", "anime"]).optional(),
    controlnet: z.object({
  ...
  providers: [
    { id: "gemini-3.1-pro-image", provider: "google",    model: "gemini-3.1-pro-image",  tier: "premium",  cost_per_call: 0.04,  avg_latency_ms: 8000  },
    { id: "gemini-3.1-flash-image", provider: "google",    model: "gemini-3.1-flash-image", tier: "standard", cost_per_call: 0.02,  avg_latency_ms: 4000  },
    { id: "gemini-3.1-pro",      provider: "google",    model: "gemini-3.1-pro", tier: "premium",  cost_per_call: 0.03,  avg_latency_ms: 6000  },
  ],
  // ...
}
```
...
const videoTrimAdapter: PrimitiveAdapter<VideoTrimInput, VideoOutput> = {
  ...
  domain: "video",
  providers: [
    { id: "local-ffmpeg", provider: "self-hosted", model: "ffmpeg", tier: "standard", cost_per_call: 0, avg_latency_ms: 2000 }
  ],
...
for (const group of parallelGroups) {
  if (group.length === 1) {
    ...
    group.forEach((step, i) => { results[step.id] = groupResults[i] })
  }
}
```
