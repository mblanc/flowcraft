 ...
 *
 * @param gcsUri The GCS URI to validate (e.g., "gs://bucket-name/path/to/file").
 * @returns An object containing the bucket name and file path.
 * @throws Error if the URI is invalid or doesn't belong to the configured bucket.
 */
export function validateAndParseGcsUri(gcsUri: string): {
    ...
    const expectedBucketName = storageUri.startsWith("gs://")
        ? storageUri.substring(5).split("/")[0]
        : storageUri.split("/")[0];

    if (bucketName !== expectedBucketName) {
        logger.error(
            `Security alert: Unauthorized bucket access attempt: ${bucketName}`,
        );
