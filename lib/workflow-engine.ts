...
export class WorkflowEngine {
    ...
    private gatherInputs(node: Node<NodeData>): NodeInputs {
        ...
        if (node.data.type === "agent") {
            // Agent inputs
            const promptEdge = incomingEdges.find(
                (edge) => edge.targetHandle === "prompt-input",
            );
            if (promptEdge) {
                const sourceData = getSourceData(promptEdge.source);
                if (sourceData?.type === "text")
                    inputs.prompt = sourceData.text;
            }

            const fileEdges = incomingEdges.filter(
                (edge) => edge.targetHandle === "file-input",
            );
            inputs.files = [];
            ...
            const promptEdge = incomingEdges.find(
                (edge) =>
                    !edge.targetHandle || edge.targetHandle !== "image-input",
            );
            if (promptEdge) {
                const sourceData = getSourceData(promptEdge.source);
                if (sourceData?.type === "text")
            ...
            const firstFrameEdge = incomingEdges.find(
                (edge) => edge.targetHandle === "first-frame-input",
            );
            if (firstFrameEdge) {
                const sourceData = getSourceData(firstFrameEdge.source);
                if (sourceData?.type === "image" && sourceData.images?.[0])
                    inputs.firstFrame = sourceData.images[0];
                else if (
                    sourceData?.type === "file" &&
                    sourceData.fileType === "image" &&
                    sourceData.gcsUri
                )
                    inputs.firstFrame = sourceData.gcsUri;
                else if (sourceData?.type === "upscale" && sourceData.image)
                    inputs.firstFrame = sourceData.image;
                else if (sourceData?.type === "resize" && sourceData.output)
                    inputs.firstFrame = sourceData.output;
            }

            const lastFrameEdge = incomingEdges.find(
                (edge) => edge.targetHandle === "last-frame-input",
            );
            if (lastFrameEdge) {
                const sourceData = getSourceData(lastFrameEdge.source);
                if (sourceData?.type === "image" && sourceData.images?.[0])
                    inputs.lastFrame = sourceData.images[0];
                else if (
                    sourceData?.type === "file" &&
                    sourceData.fileType === "image" &&
                    sourceData.gcsUri
                )
                    inputs.lastFrame = sourceData.gcsUri;
                else if (sourceData?.type === "upscale" && sourceData.image)
                    inputs.lastFrame = sourceData.image;
                else if (sourceData?.type === "resize" && sourceData.output)
                    inputs.lastFrame = sourceData.output;
            }
            ...
            if (imageEdge) {
                const sourceData = getSourceData(imageEdge.source);
                console.log(
                    "[WorkflowEngine] Resize Node Source Data:",
                ...
                if (sourceData?.type === "image" && sourceData.images?.[0]) {
                    ...
                    if (sourceData.fileType === "image" && sourceData.gcsUri) {
                        ...
                        console.warn(
                            "[WorkflowEngine] File Node source is not a valid image:",
                            {
                ...
                console.log(
                    "[WorkflowEngine] Resize Node Resolved Input:",
                    inputs.image,
                );
            } else {
                console.warn(
                    "[WorkflowEngine] No image edge found for resize node",
                );
            }
        }
